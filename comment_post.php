...
header("Content-Type:text/html; charset=utf-8");
 		 ...
 		 include("loginheader.php");?>	<script type="text/javascript" src="../lib/jquery-1.8.2.min.js"></script>
...
<script type ="text/javascript" src="source/helpers/jquery.fancybox-media.js?v=1.0.5"></script>
	...
	</style> <?php if($checkSessionExist > 0)
 {
	 		if(isset($_GET['commentID']))
		{
			$postID=$_GET['commentID'];
			$sql="SELECT COUNT(*) AS CommentExist FROM Comment WHERE CommentID=" . $postID;
		$conn=odbc_connect('sba','','',SQL_CUR_USE_ODBC);
		$cmrs=odbc_exec($conn,$sql);
		$commentExist=odbc_result($cmrs,"CommentExist");
			if ($commentExist==0)
			{?>
	...
	$username = CheckUserName ($sessionID);	 ?>
      ...
      <div class="SignUpLoginTopic">Comment<br></div>
       ...
       <div class="backToTop"> <a href="comment.php" id='trans'>Back to Top</a></div>
...
$replaceTime = 0;
	...
	<div class='comment-padding'>
		...
		{	echo"
			...
			echo"style='max-height:70px;max-width:70px;  -moz-border-radius: 6px;
  -webkit-border-radius: 6px;'>
            </div>
            <div class='comment-username'>".$postUserName."</div>
			 </div>
            <div class='comment-content-detail' id='" . $id . "'>".$postContent."</div>
            <div class='comment-footer'>
            	<div class='comment-select'>
            <div class='comment-select-box'><div class='replybutton' id='" . $id . "'><a href='javascript:;'><span class='webSymbol'>&#104;</span>&nbsp;Reply (" . $replyNumber .")</div></a></div><div class='comment-select-box'><a href='comment_post.php?commentID=" . $id . "'><span class='webSymbol'>&#101;</span>&nbsp;Detail</a></div>";
		...
		//REPLY
		echo "
		<div class='reply-bigcontainer-show' id='" . $id . "'>";
		if ($replyNumber>0)
			{
